...
energy_file = "zz_energies.dat"                # Filename for the 0-0 peak energies. If left empty script will use energies from relaxations.
...
def intensity(d, freq, mu, m, n, T, S_limit, relevant=[]):
    ...
    if relevant:
        ...
        for i in relevant:
            # Get S
	    S = get_huang_rhys(np.linalg.norm(d), mu[i], freq[i])
